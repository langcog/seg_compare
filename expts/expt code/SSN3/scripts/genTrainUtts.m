...
numSents = round(totalWords / sentLen);
sentLens = ones(numSents,1)*sentLen;
numEachWord = ceil(totalWords/lex.numWds);
words = ones(1,lex.numWds)*numEachWord;
acceptableReps = ceil(sentLen/lex.numWds);
ind = 1:lex.numWds;
...
while ~completed
  ...
  if sum(abs(words)) <= 2* sentLen, completed = 1; else disp(words); end;
end;